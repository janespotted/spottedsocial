...
export default function Map() {
  ...
  return (
    <div className="relative h-screen w-full">
      ...
      {friends.length > 0 ? (
        <div ref={friendsListRef} className={`absolute left-6 z-[200] max-w-sm transition-opacity duration-300 ${focusMode ? 'opacity-0 pointer-events-none' : 'opacity-100'}`} style={{ bottom: 'calc(6rem + env(safe-area-inset-bottom))' }}>
        ...
        <div className={`absolute left-6 bg-[#2d1b4e]/90 backdrop-blur border border-[#a855f7]/30 rounded-lg p-3 z-20 transition-opacity duration-300 ${focusMode ? 'opacity-0 pointer-events-none' : 'opacity-100'}`} style={{ bottom: 'calc(6rem + env(safe-area-inset-bottom))' }}>
          <div className="flex items-center gap-2">
      ...
      {/* Legend */}
      <div className={`absolute right-6 bg-[#2d1b4e]/95 backdrop-blur-sm border border-[#a855f7]/20 rounded-md p-2 z-20 shadow-[0_0_8px_rgba(168,85,247,0.2)] transition-opacity duration-300 ${focusMode ? 'opacity-0 pointer-events-none' : 'opacity-100'}`} style={{ bottom: 'calc(10rem + env(safe-area-inset-bottom))' }}>
