...
import {
  invalidateNightStatusQueries,
  useOwnNightStatus,
} from '@/hooks/use-own-night-status';
  ...
  return (
    <View
      ...
      style={{
        ...
        borderColor: ring,
        boxShadow: selected
          ? `0 0 0 3px ${NEON}, 0 0 18px ${NEON}aa`
          : `0 0 8px ${ring}66`,
          ...
          <View style={{width: size, height: size}}>
            ...
            {extra > 0 ? (
              <View className='absolute -bottom-1 -right-1 min-w-5 h-5 px-1 rounded-full bg-[#1a0f2e] border border-white/30 items-center justify-center'>
                <Text className='text-white text-[11px] font-sans-semibold'>
                  +{extra}
                </Text>
              </View>
            ) : null}
...
export default function MapScreen() {
  ...
  // Open on the user, not the city centre. Only when permission already
  // exists — this must never prompt — and only once per mount, so it can't
  // yank the camera back while someone is panning around. Falls back to the
  // city centre, which is what defaultSettings already drew.
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      if (!hasLocationAccess(await getLocationPermission())) return;
    ...
    if (self && shouldCluster) {
      for (const cluster of groups) {
        ...
        if (sameVenue || closeGps) {
          ...
          if (existing >= 0) cluster.splice(existing, 1);
  ...
  return (
    <View className='flex-1 bg-[#110a24]'>
      ...
      {!focusMode ? (
        <View
          className='absolute top-safe-offset-3 left-4 right-4 gap-3'
            ...
            >
              ...
              {city ? (
                <View className='px-2.5 py-1 rounded-full bg-[#1a0f2e]/85 border border-white/10'>
                  <Text className='text-white/70 text-xs font-sans-medium uppercase'>
                    {city}
                  </Text>
                </View>
              ) : null}
          ...
          <View className='flex-row items-center gap-2'>
            ...
            >
              <SymbolView
                name='person.2.fill'
                size={13}
                tintColor={friendsOnly ? NEON : 'rgba(255,255,255,0.7)'}
              />
              <Text
                className='text-xs font-sans-medium'
                style={{color: friendsOnly ? NEON : 'rgba(255,255,255,0.7)'}}
              >
                Friends
              </Text>
            </Pressable>
      ...
      {!focusMode ? (
        <View
          className='absolute bottom-safe-offset-28 left-16 right-16 items-center'
          pointerEvents='box-none'
        >
          {myStatus?.status === 'out' ? (
            <View className='flex-row items-center gap-1 pl-4 pr-1 py-1 rounded-full bg-[#1a0f2e]/95 border border-[#d4ff00]/30'>
              ...
              >
                ...
                <Text className='text-white/50 text-[11px] font-sans'>
                  {RESET_COPY.liveUntil}
                </Text>
              </Pressable>
              <Pressable
                onPress={async () => {
              ...
              >
                <Text className='text-red-300 text-sm font-sans-medium'>
                  Stop sharing
                </Text>
              </Pressable>
            </View>
            ...
            <Pressable
              ...
              style={{
                ...
                boxShadow: '0 4px 16px rgba(212,255,0,0.3)',
              }}
            >
              <Text className='text-[#1a0f2e] text-sm font-sans-semibold'>
                Update status
              </Text>
            </Pressable>
          )}
