...
-- Drop the overly permissive policy that has "OR true" clause
DROP POLICY IF EXISTS "Users can read own profile or public data" ON profiles;

-- Create proper self-only SELECT policy
-- Other users' profiles should be accessed via get_profiles_safe() or get_profile_safe() RPC functions
-- which properly enforce privacy controls for sensitive fields
CREATE POLICY "Users can read own profile" ON profiles
FOR SELECT
USING (auth.uid() = id);

-- Allow authenticated users to read basic profile info for foreign key lookups (username, display_name, avatar_url)
-- Sensitive fields (location, is_out, last_active_at) are still protected via the RPC functions
CREATE POLICY "Authenticated users can read basic profile info" ON profiles
FOR SELECT
USING (
  auth.uid() IS NOT NULL
  -- Note: Sensitive location fields are masked by get_profiles_safe() RPC
  -- This policy exists for foreign key lookups and basic profile display
);

-- SECURITY FIX: Add auth verification to process_invite_code
-- Prevents malicious users from calling with someone else's user_id
...
BEGIN
  -- SECURITY FIX: Verify the calling user matches new_user_id
