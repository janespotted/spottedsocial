...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'seed') {
      ...
      // Night statuses
      await sb.from('night_statuses').insert(uids.slice(0,8).map((u,i)=>{const v=V[i%V.length];return{user_id:u,status:'out',venue_id:vm.get(v.name),venue_name:v.name,lat:v.lat,lng:v.lng,expires_at:exp(),is_demo:true};}));
